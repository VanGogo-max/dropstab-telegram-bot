# Файл: formatter.py
# Telegram форматиране на анализи
...
class TelegramFormatter:
    ...
    def format_analysis(sentiment, opportunities, lang='en'):
        """Generate Telegram-formatted analysis"""
        ...
        if opportunities:
            ...
            post += "No opportunities matching criteria\n\n"
        ...
        if opportunities:
            ...
            if oversold:
                symbols = ' | '.join([f"`{c.get('symbol', '')}`" for c in oversold[:5]])
                post += f"{t['oversold']}:\n{symbols}\n\n"
